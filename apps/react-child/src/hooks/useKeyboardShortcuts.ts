...
export function useKeyboardShortcuts(options: ShortcutsOptions = {}) {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      const { onNewChat, onToggleSidebar, onFocusInput, onSearchFocus } =
        optionsRef.current;
      ...
      const actions: Record<string, () => void> = {
        newChat: onNewChat ?? (() => dispatch(chatActions.createConversation({ model: selectedModel }))),
        focusInput: onFocusInput ?? (() => {
          const input = document.querySelector('textarea[placeholder="发送消息..."]') as HTMLTextAreaElement;
          input?.focus();
        }),
        focusSearch: onSearchFocus ?? (() => {
          const search = document.querySelector('input[placeholder="搜索对话..."]') as HTMLInputElement;
          search?.focus();
        }),
